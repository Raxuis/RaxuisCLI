...
func printRow(cells []string, widths []int, isHeader bool) {
	...
	fmt.Fprintln(stdoutW, "  "+strings.Join(parts, "  "))
}
...
func PrintKeyValueIndent(key, value string, indent int) {
	...
	fmt.Fprintf(stdoutW, "%s%s%-15s%s %s\n", indentStr, BoldCyan, key+":", Reset, value)
}

// PrintSection prints a section header
func PrintSection(title string) {
	fmt.Fprintf(stdoutW, "\n%s[%s]%s\n", BoldWhite, title, Reset)
	fmt.Fprintln(stdoutW, strings.Repeat("-", len(title)+2))
}
...
func PrintHeader(title string) {
	...
	fmt.Fprintln(stdoutW, strings.Repeat("=", width))
	padding := (width - len(title)) / 2
	fmt.Fprintf(stdoutW, "%s%s%s%s\n", strings.Repeat(" ", padding), BoldWhite, title, Reset)
	fmt.Fprintln(stdoutW, strings.Repeat("=", width))
}
...
func PrintSubHeader(title string) {
	...
	fmt.Fprintln(stdoutW, strings.Repeat("-", len(title)))
}
